...
{
    var eventable_namespace = getEventable_namespace(wizard);

    // Remove virtual data objects from the wizard
    wizard.unbind(eventable_namespace).removeData("uid").removeData("options")
        .removeData("state").removeData("steps").removeData("eventable_namespace")
        .find(".actions a").unbind(eventable_namespace);
 ...
 * @static
 * @private
 * @method getEventable_namespace
 * @param wizard {Object} A jQuery wizard object
 * @return {String} Returns the unique event namespace for the given wizard
 */
function getEventable_namespace(wizard)
{
    var eventable_namespace = wizard.data("eventable_namespace");

    if (eventable_namespace == null)
    {
        eventable_namespace = "." + getUniqueId(wizard);
        wizard.data("eventable_namespace", eventable_namespace);
    }

    return eventable_namespace;
}
...
function registerEvents(wizard, options)
{
    ...
    wizard.bind("finishing" + eventable_namespace, options.onFinishing);
    wizard.bind("finished" + eventable_namespace, options.onFinished);
    wizard.bind("stepChanging" + eventable_namespace, options.onStepChanging);
    wizard.bind("stepChanged" + eventable_namespace, options.onStepChanged);

    if (options.enableKeyNavigation)
    {
        wizard.bind("keyup" + eventable_namespace, keyUpHandler);
    }

    wizard.find(".actions a").bind("click" + eventable_namespace, paginationClickHandler);
}
...
function renderTitle(wizard, options, state, header, index)
{
    ...
    stepItem.children("a").bind("click" + getEventable_namespace(wizard), stepClickHandler);
}
 ...
 * @param argumentable_name {String} The name of the given argument
 * @param argumentValue {Object} The argument itself
 */
function validateArgument(argumentable_name, argumentValue)
{
    if (argumentValue == null)
    {
        throwError("The argument '{0}' is null or undefined.", argumentable_name);
    }
}
